...
export class RestaurantProfileComponent implements OnInit, OnDestroy {
  ...
  loadProfile(id: number): void {
    ...
    this.businessService.getPublicProfile(id).subscribe({
      next: (profile) => {
        ...
        // ✅ Guard : on ne track que si l'id est défini
        if (profile.id) {
  ...
  goToReservation(): void {
    ...
    if (this.profile.id) {
      this.analyticsService.trackReservationStarted(this.profile.id);
    }

    sessionStorage.setItem('reservation_restaurant', JSON.stringify({
      id:           this.profile.id,
      name:         this.profile.name,
      type:         this.profile.type,
      address:      this.profile.address,
      phone:        this.profile.phone,
      opening_hour: this.profile.opening_hour,
      closing_hour: this.profile.closing_hour,
      image_url:    this.profile.image_url,
      requires_reservation_deposit: this.profile.requires_reservation_deposit,
      default_deposit_amount:       this.profile.default_deposit_amount,
    }));
  ...
  // ✅ Tracker les clics sur les articles du menu
  ...
  isOpen(): boolean {
    const b = this.profile;
    if (!b) return false;
    if (b.type !== 'restaurant') return b.is_available ?? false;
    if (!b.opening_hour || !b.closing_hour) return false;
    ...
    const open  = toMin(b.opening_hour);
    const close = toMin(b.closing_hour);
